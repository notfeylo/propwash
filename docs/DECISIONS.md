...
## 2026-09-26 · Cameras (§7)

- `C` cycles Orbit → FPV → Chase → LOS → HD. The outside views (Orbit, Chase, LOS) render clean and full screen with no OSD; FPV and HD keep their video looks.
- **Chase** is a critically damped spring (ω = 6 rad/s) toward a point 2.4 m behind by heading and 0.8 m up. It is damped toward the drone's own motion as seen per rendered second, so steady flight leaves no lag and a slow or paused replay doesn't run ahead. It aims 0.22 s ahead along that motion. No-clip: a Rapier ray from the drone to the camera (ignoring the drone's own colliders) keeps it in front of whatever is in between, and it never goes below 0.35 m above the ground.
- **LOS** is the pilot standing 5 m behind the pad, eyes at 1.7 m, turning toward the drone (80 ms smoothing) with auto-zoom: the FOV keeps a 0.6 m subject at 10% of the frame height, clamped to 5–60°, smoothed over 0.6 s. Field trees can block the view, as they would.
- **HD stabilization** (K, or D-pad → on a pad; also in Settings → Camera): Raw, Smooth (the camera orientation low-passed at 0.18 s, like HyperSmooth) and Horizon lock (the smoothed view direction with world up, so roll is removed). Both stabilized modes crop in to 80% of the FOV, as Gyroflow and HyperSmooth do to hide the rotated edges. Horizon lock is the default. Pointing within 14° of straight up or down the level is undefined, and it falls back to the smoothed view. J was the first choice of key, but the rebind test uses it for arming.
- **Replay** is not a separate camera: during a replay every view works on the replayed drone, and the orbit view is the free camera (drag, zoom, pan, follow).

## 2026-09-26 · Recording and deterministic replay (§8.2)

- A recording starts when the drone arms and ends 3 s after the disarm, so the landing or crash is in it; it is capped at 10 minutes. It holds a **checkpoint** (Rapier's `takeSnapshot` plus a copy of every mutable field of our own sim: motors, pack, wind, FC filters and integrators, prop wash, autoland and all RNG streams), the **op log** and the blackbox.
- The op log holds every change from outside, stamped with the step it came before: inputs (logged when the object changes, once per frame), arm (with the inputs of that moment, which the FC's stick filters start from), pad reset, airframe swap (payload), FC settings (mode, rates, PIDs, sensors: now applied through `FlightSim.configureFc`), battery plug and land-mode start. `Rng` keeps its state in a field instead of a closure so it can be copied.
- A **replay** builds a second sim with the same options, restores the checkpoint and applies the log. It resimulates ahead of playback for up to 6 ms per frame (≈ 40× real time headless), and its own blackbox is compared with the recording's as it goes: the replay bar says "bit-identical" or shows the first time they differ. Headless: 5.6 s with a mode switch, payload swap, disarm, pad reset and re-arm, identical to the last bit.
- Not logged, so they would break a replay: the debug teleport (`placeDrone`) and changes to global config objects mid-flight (sensor noise settings). Neither is reachable while flying normally.
- Video export records the WebGL/WebGPU canvas with `MediaRecorder` (WebM, 12 Mb/s) while the replay plays. The OSD is a separate overlay canvas and isn't in the video, and there is no audio track yet.

## 2026-09-26 · Blackbox and CSV (§8.2)

- 500 Hz (every other 1 kHz step, like Betaflight's 1/2 rate at 1 kHz), 50 channels as float32, in one-second chunks: sticks, setpoints, filtered and raw gyro, P/I/D/F per axis, motor commands, RPM, attitude quaternion and the FC's estimate, position, velocity, voltage, current, mAh. About 100 KB/s.
- The CSV follows `blackbox_decode`'s column names and, where they mean the same thing, its units: `time (us)`, `rcCommand[0–2]` ±500 and `rcCommand[3]` 1000–2000, `setpoint[0–2]` in °/s and `setpoint[3]` 0–1000, `gyroADC` / `gyroUnfilt` in °/s, `motor[n]` as DShot values 48–2047, `eRPM[n]` in eRPM/100, `vbatLatest (V)`, `amperageLatest (A)`, `energyCumulative (mAh)`. `axisP/I/D/F` are our PID terms in °/s² (the PID outputs angular acceleration), not Betaflight's internal scale. Extra columns at the end carry the sim's truth: `rpm`, `attitude`, `quat`, `pos`, `vel`. The seed and airframe go in the file name.

## 2026-09-26 · Step response and log import (§8.2–§8.3)

- The step response is a port of PIDtoolbox's `PTstepcalc`: 2 s Hann-windowed segments every 100 ms, zero-padded, Wiener deconvolution of gyro by setpoint on length-normalised spectra (λ = 10⁻⁴), integrated to a step, each scaled to settle at 1 over 200–500 ms and kept only if it stayed between 0.5 and 3 there, then averaged. Checked against a known system (6 ms delay + 20 ms lag): 50% at 18 ms (exact 19.9), 90% at 50 ms (exact 52.1).
